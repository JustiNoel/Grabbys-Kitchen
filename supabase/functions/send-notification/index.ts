...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (type === "order") {
      ...
      customerSubject = "Order Confirmation - Savanna Kitchen";
      customerHtml = `
        <div style="font-family: 'DM Sans', Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #faf8f5; padding: 20px;">
          <div style="background: linear-gradient(135deg, #d4652a 0%, #e8a64c 100%); padding: 30px; text-align: center; border-radius: 12px 12px 0 0;">
            <h1 style="color: white; margin: 0; font-family: 'Playfair Display', serif;">Savanna Kitchen</h1>
          </div>
      ...
      customerSubject = "Reservation Confirmation - Savanna Kitchen";
      customerHtml = `
        <div style="font-family: 'DM Sans', Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #faf8f5; padding: 20px;">
          <div style="background: linear-gradient(135deg, #d4652a 0%, #e8a64c 100%); padding: 30px; text-align: center; border-radius: 12px 12px 0 0;">
            <h1 style="color: white; margin: 0; font-family: 'Playfair Display', serif;">Savanna Kitchen</h1>
    ...
    const customerResponse = await resend.emails.send({
      from: "Savanna Kitchen <onboarding@resend.dev>",
      to: [customerEmail],
